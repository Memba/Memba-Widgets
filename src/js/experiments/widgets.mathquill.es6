...
// TODO Allow \\MathQuillMathField in the field - https://github.com/kidoju/Kidoju-Widgets/issues/195
...
import './widgets.mathquill.toolbar.es6';
...
const {
    ...
    notify,
    roleSelector,
    ui: { MathQuillToolBar, plugin, Widget },
    unbind,
} = window.kendo;
const logger = new Logger('widgets.mathquill');
const WIDGET = 'kendoMathQuill';
const NS = CONSTANTS.DOT + WIDGET;
const WIDGET_CLASS = 'kj-mathquill'; // 'k-widget kj-mathquill';
...
const MathQuill = Widget.extend({
    ...
    _onFocusIn(e) {
        ...
        if (container.length) {
            // $(document).find(roleSelector('mathquilltoolbar')).hide();
            container.children(roleSelector('mathquilltoolbar')).hide();
            // Show widget's toolbar
            if (
                this._activeField instanceof MQ.MathField &&
                toolBar instanceof MathQuillToolBar
            ) {
                setTimeout(() => {
        ...
        if (toolBar instanceof MathQuillToolBar) {
            toolBar.resize();
        }
    ...
    _openDialog(name, options) {
        const dialog = window.kendo.mathquill.dialogs.create(name, options);
    ...
    destroy() {
        ...
        unbind(wrapper);
        // Release references
        if (that.toolBar instanceof MathQuillToolBar) {
 ...
 * Registration
 */
if (!Object.prototype.hasOwnProperty.call(window.kendo.ui, 'MathQuill')) {
