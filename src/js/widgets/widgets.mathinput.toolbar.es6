...
window.kendo.mathinput = { messages: {} };
...
mathinput.messages.toolbar = TOOLBAR_MESSAGES;
const toolDefaults = {
 ...
 * MathInputToolBar
 */
const MathInputToolBar = ToolBar.extend({
    init(element, options = {}) {
        // eslint-disable-next-line no-param-reassign
        options.items = this._expandTools(
            options.tools || MathInputToolBar.prototype.options.tools
        );
 ...
 */
if (
    !Object.prototype.hasOwnProperty.call(window.kendo.ui, 'MathInputToolBar')
) {
...
const KeyPadTool = PopupTool.extend({
    ...
    _commandPalette() {
        const { buttons } = this;
        const element = $('<div class="kj-mathinput-palette" />').appendTo(
            this.popup.element
        );
...
const DIALOG_MESSAGES = mathinput.messages.dialogs;
 ...
 * MathInputDialog
 */
const MathInputDialog = Observable.extend({
    init(options) {
...
const KeyPadDialog = MathInputDialog.extend({
    init(options) {
        const messages = mathinput.messages.dialogs || DIALOG_MESSAGES;
...
const ExpressionsDialog = MathInputDialog.extend({
    init(options) {
        const messages =
            mathinput.messages.dialogs.expression || DIALOG_MESSAGES;
        const defaultOptions = {
...
const SetsDialog = MathInputDialog.extend({
    init(options) {
        const messages = mathinput.messages.dialogs.set || DIALOG_MESSAGES;
        const defaultOptions = {
            title: messages.title,
            buttons: [
                // See also https://texblog.org/2007/08/27/number-sets-prime-natural-integer-rational-real-and-complex-in-latex/
...
const MatricesDialog = MathInputDialog.extend({
    init(options) {
        const messages = mathinput.messages.dialogs.matrices || DIALOG_MESSAGES;
        const defaultOptions = {
            title: messages.title,
            buttons: [
                // https://github.com/mathquill/mathquill/issues/332
...
const StatisticsDialog = MathInputDialog.extend({
    init(options) {
        const messages =
            mathinput.messages.dialogs.statistics || DIALOG_MESSAGES;
        const defaultOptions = {
            title: messages.title,
            buttons: [
                {
...
var ChemistryDialog = MathInputDialog.extend({
    init: function (options) {
        var messages = mathinput.messages.dialogs.chemistry || DIALOG_MESSAGES;
        var defaultOptions = {
            title: messages.title,
            buttons: [
                // TODO
...
mathinput.dialogs.register('chemistry', ChemistryDialog);
 */
