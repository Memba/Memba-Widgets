...
// Component data
import { getNumericBox } from '../../../src/js/helpers/helpers.components.es6';
    ...
    describe('NumericBoxTool', () => {
        ...
        it('It should have descriptors', () => {
            ...
            expect(tool).to.have.property(
                'description',
                __('tools.numericbox.description')
            );
            expect(tool).to.have.property('height', 80);
            expect(tool).to.have.property('help', __('tools.numericbox.help'));
            expect(tool).to.have.property('id', TOOL);
            expect(tool).to.have.property('icon', __('tools.numericbox.icon'));
            expect(tool)
                .to.have.property('menu')
                .that.eql(['properties.question', 'properties.solution']);
            expect(tool).to.have.property('name', __('tools.numericbox.name'));
